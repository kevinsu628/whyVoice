...
def print_eval(name, scores, labels, loss, end="\n", cm=False):
    ...
    for t, p in zip(labels.data, torch.max(scores, 1)[1].view(batch_size).data):
...
def evaluate(config, model=None, test_loader=None):
    ...
    for model_in, labels in test_loader:
        ...
        results.append(print_eval("test", scores, labels, loss, cm=True) * model_in.size(0))
    ...
    for epoch_idx in range(config["n_epochs"]):
        ...
        if epoch_idx % config["dev_every"] == config["dev_every"] - 1:
            ...
            for model_in, labels in dev_loader:
                ...
                labels = Variable(labels, requires_grad=False)
                loss = criterion(scores, labels)
                accs.append(print_eval("dev", scores, labels, loss, cm=True))
